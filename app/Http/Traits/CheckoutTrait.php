...
trait CheckoutTrait
{
  ...
  public function getStateTaxRate($request)
  {
    $stateId = null;
    
    // Try to get state from billing_address_id first
    if (isset($request->billing_address_id) && $request->billing_address_id) {
      $address = Helpers::getAddressById($request->billing_address_id);
      if ($address && isset($address['state_id'])) {
        $stateId = $address['state_id'];
      }
    }
    
    // If not found, try billing_address array (for guest checkout)
    if (!$stateId && isset($request->billing_address) && is_array($request->billing_address)) {
      if (isset($request->billing_address['state_id'])) {
        $stateId = $request->billing_address['state_id'];
      }
    }
    
    // If still not found, try shipping address
    if (!$stateId && isset($request->shipping_address_id) && $request->shipping_address_id) {
      $address = Helpers::getAddressById($request->shipping_address_id);
      if ($address && isset($address['state_id'])) {
        $stateId = $address['state_id'];
      }
    }
    
    // Try shipping_address array
    if (!$stateId && isset($request->shipping_address) && is_array($request->shipping_address)) {
      if (isset($request->shipping_address['state_id'])) {
  ...
  public function calculateCosts($request, $shippingRules)
  {
    try {
      ...
      foreach ($request->products as $product) {
        ...
        $perProductTax = $this->getTax($product['product_id'], $subTotal, $request);
        $tax[] = $perProductTax;
        $perProductCost[] = [
  ...
  public function getTax($product_id, $subtotal, $request = null)
  {
    $tax = 0;
    $taxRate = null;
    
    // First, try to get state-based tax rate
    if ($request) {
      $stateTaxRate = $this->getStateTaxRate($request);
      if ($stateTaxRate !== null) {
        $taxRate = $stateTaxRate;
      }
    }
    
    // If no state tax found, fall back to product tax
    if ($taxRate === null) {
