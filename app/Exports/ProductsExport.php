...
class ProductsExport implements FromQuery, WithMapping, WithHeadings
{
    ...
    public function map($product): array
    {
        ...
        $result = array_merge(array_splice($rows, 0, 1), $translateValues, $rows);

        // Free loaded relations from memory after mapping this row
    ...
    /**
     * Centralized list of relations to load per product
     */
    private function getRelations(): array
    {
        return [
            'variations.variation_image',
            'variations.digital_files',
    ...
    public function getVariation($variations)
    {
        ...
        foreach ($variations as $variation) {
            ...
            $formattedVariation = [
                ...
                'attribute_values'       => $variation?->attribute_values?->pluck('id')?->implode(','),
                ...
                'variation_galleries_url'=> $variation?->variation_galleries?->pluck('original_url')?->implode(','),
    ...
    public function filter($product, $request)
    {
        ...
        if (!empty($request->field) && !empty($request->sort)) {
    ...
    function rows($product)
    {
        return [
            ...
            'wholesale_prices'            => $product->wholesales ? json_encode($product->wholesales->toArray()) : '',
            'license_key'                 => $product->license_keys?->pluck('license_key')->implode(','),
            'variations'                  => $product->variations ? json_encode($this->getVariation($product->variations)) : '',
            'external_details'            => $product->external_details ? json_encode($product->external_details) : '',
    ...
    function getTranslatedValues($product)
    {
        ...
        // Filter out Spanish locale
        $locale = array_filter($locale, function ($localeName) {
